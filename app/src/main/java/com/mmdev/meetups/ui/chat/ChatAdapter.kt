...
class ChatAdapter constructor(options: FirestoreRecyclerOptions<ChatModel>, private val mUserName: String,
	...
	override fun onBindViewHolder(viewHolder: ChatViewHolder, position: Int, chatModel: ChatModel) {
		...
		chatModel.fileModel?.url?. let { viewHolder.setIvChatPhoto(it) }
	...
	override fun getItemViewType(position: Int): Int {
		val (senderUserModel, _, fileModel, mapModel) = getItem(position)
		return if (mapModel != null) {
			if (senderUserModel!!.name == mUserName) RIGHT_MSG_IMG
			else LEFT_MSG_IMG
		} else
			if (fileModel != null) {
			if (fileModel.type == "img" && senderUserModel!!.name == mUserName) RIGHT_MSG_IMG
			else LEFT_MSG_IMG
		} else
